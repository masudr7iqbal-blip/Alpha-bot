...
def welcome(message):
    if is_subscribed(message.from_user.id):
        ...
        bot.send_message(message.chat.id, f"স্বাগতম {message.from_user.first_name}! এক্সেস আনলক হয়েছে।", reply_markup=markup)
    else:
        markup = types.InlineKeyboardMarkup()
...
def handle(call):
    if call.data == "verify":
        if is_subscribed(call.from_user.id):
            ...
            bot.answer_callback_query(call.id, "⚠️ আগে সবগুলোতে জয়েন করুন!", show_alert=True)
